...
public class MongoDecoder
{
    ...
    public Map<String,String> readMap( String pAttribute ) {
        Document tDoc = (Document) mDoc.get( pAttribute );
        return docMapToJavaMap( tDoc );
    }
    ...
    public List<Map<String,String>> readMapArray( String pAttribute) {
       List<Document> tMapDocLst = (List<Document>) mDoc.get( pAttribute );
       if (tMapDocLst == null) {
           return null;
       }
       List<Map<String,String>> tMapLst = new ArrayList<>();
       for( Document tDoc : tMapDocLst) {
           tMapLst.add( docMapToJavaMap( tDoc ));
       }
       return tMapLst;
    }
    ...
    private Map<String,String> docMapToJavaMap( Document pDocMap ) {
        Map<String,String> tMap = new HashMap<>();
        for( Map.Entry<String,Object> tEntry : pDocMap.entrySet()) {
            tMap.put(tEntry.getKey(), tEntry.getValue().toString());
        }
        return tMap;
    }
