...
public class TestMongoSupportTest {
    ...
    public void test() {
        ...
        assertEquals(dtmLst.size(), 1);
        assertEquals( DateUtils.localDateToString(dtmLst.get(0).getDate().get()), tDateString );
        assertEquals( DateUtils.localDateTimeToString(dtmLst.get(0).getDateTime().get()), tDateTimeString );
        ...
        assertEquals(dtmLst.size(), 1);
        assertEquals(tDateString, DateUtils.localDateToString(dtmLst.get(0).getDate().get()));
        assertEquals(tDateTimeString, DateUtils.localDateTimeToString(dtmLst.get(0).getDateTime().get()));
        ...
        assertEquals(tMsgs.size(), 11);

        List<TestMessage> tml = mDbAux.findTestMessage(102, "key-102");
        assertEquals(tml.size(), 1);
        assertEquals( (int) tml.get(0).getIntValue().get(), (int) 102 );
        assertEquals( tml.get(0).getStrValue().get(), "key-102" );

        tml = mDbAux.findTestMessageByIntValue(103);
        assertEquals(tml.size(), 1);
        assertEquals( (int) tml.get(0).getIntValue().get(), (int) 103 );
        ...
        tml = mDbAux.findTestMessage(102, "key-102");
        testMsg = tml.get(0);
        assertEquals(testMsg.getTimeString().get(), "1998-06-11 16:36:32.000");
        ...
        assertEquals(tRes.getMatchedCount(), 1 );
        assertEquals(tRes.getModifiedCount(), 1 );

        testMsg.setIntValue(666);
        tRes = mDbAux.updateTestMessage(testMsg, true);
        assertEquals(tRes.getMatchedCount(), 0 );
        assertNotNull(tRes.getUpsertedId());
        assertEquals(tRes.getModifiedCount(), 0 );


        // Test Delete Method
        mDbAux.deleteTestMessageByIntValue( 102 );
        tml = mDbAux.findTestMessageByIntValue(102);
        assertEquals( tml.size(), 0);
